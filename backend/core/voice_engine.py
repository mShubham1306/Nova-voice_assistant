...
# Try to import alternate audio backend (sounddevice) to avoid PyAudio compilation issues
try:
    ...
    _SR_AVAILABLE = False
    print("[Voice] speech_recognition, sounddevice, or soundfile not available. Voice input disabled.")
...
class VoiceEngine:
    ...
    Works in degraded mode if sounddevice/pyttsx3 are missing.
    """
    ...
    def __init__(self, socketio=None):
        ...
        if _SR_AVAILABLE:
            try:
                ...
                # Test microphone access via sounddevice
                sd.default.device
                self.mic_available = True
                print("[Voice] Microphone ready (using sounddevice proxy).")
